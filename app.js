...
function initDeepResearchGenerator() {
    ...
    const updatePrompt = () => {
        ...
        const rawPrompt = `내가 기획하고자 하는 사업 주제는 "${state.currentTopic}"이고, 사업을 기획 및 제안하는 신청 기관은 "${appOrg}"이며, 사업계획서를 제출하여 평가받을 심사 기관은 "${evalOrg}"야. 이 프로포절의 유형은 [${typeName}]이며, 총 사업 예산 한도는 ${budget}이고, 사업 대상자들의 구체적인 현황을 조사할 주요 대상 지역은 "${region}"이야.
...
3. [예산 편성 전략]: 해당 예산 한도 내에서 감점을 피하기 위해 반드시 지켜야 할 주요 비목 구성 및 예산 편성 유의점.

서류 작성용 세부 본문 텍스트가 아닌, 기획 방향의 뼈대를 잡기 위한 "전략적 핵심 가이드라인" 형태로 요약해서 제시해줘.`;

        // Render in UI with color code highlights
        const uiText = `내가 기획하고자 하는 사업 주제는 "${topicSpan}"이고, 사업을 기획 및 제안하는 신청 기관은 "${appOrgSpan}"이며, 사업계획서를 제출하여 평가받을 심사 기관은 "${evalOrgSpan}"야. 이 프로포절의 유형은 [${typeSpan}]이며, 총 사업 예산 한도는 ${budgetSpan}이고, 사업 대상자들의 구체적인 현황을 조사할 주요 대상 지역은 "${regionSpan}"이야.
